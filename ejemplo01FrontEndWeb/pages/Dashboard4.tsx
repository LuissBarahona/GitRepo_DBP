...
function Dashboard3() {
  ...
  const handleCreate = async () => {
    if (newProduct && newProduct.name && newProduct.price) {
      try {
        const response = await axios.post(`${BACKEND_URL}/products`, newProduct);
        setProducts([...products, response.data]); // Actualizar el estado con el producto creado
        setNewProduct(null);
      } catch (error) {
